import {useServicesHooks} from './hooks/useServicesHooks'
...
const Services = () => {
  ...
  const {deleteService, response, handleImageChange, handleTitleChange, handleUpload, deleteStatus,loading} =
    useServicesHooks()
                      ...
                      <th>
                        <div className='d-flex gap-3 justify-content-center'>
                          <button
                            ...
                            onClick={() => navigate(`/updateServices/${e.id}`)}
                          >
                            تعديل
                          </button>
                          <button className='btn btn-danger' onClick={() => deleteService(e.id)}>
                            {deleteStatus ?  <>
                              <Spinner animation="border" size="sm"></Spinner> ...يتم الحذف
                            </> : "حذف"}
                          </button>
                        </div>
                      </th>
